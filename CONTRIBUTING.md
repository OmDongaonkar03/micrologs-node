# Contributing to @micrologs/node
...
- No dependencies - the SDK must stay zero-dependency
- All methods must return `null` on failure, never throw
- Never log anything other than `console.warn` - no `console.log`, no `console.error`
- Private fields stay private - `#host` and `#key` must never be exposed on the instance
- Keep Node.js 18+ as the minimum - no polyfills, no build step
...
- New engine endpoint added → minor bump (1.0.0 → 1.1.0)
- Bug fix in SDK code → patch bump (1.0.0 → 1.0.1)
